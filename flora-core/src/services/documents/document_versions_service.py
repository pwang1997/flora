from repositories.documents import document_versions_repository
...
async def get_document_version_by_number(
    ...
    return await document_versions_repository.get_document_version_by_number(db, document_id, version_number)


async def get_latest_document_version(db: AsyncSession, document_id: str) -> DocumentVersionRecord | None:
    return await document_versions_repository.get_latest_document_version(db, document_id)
...
async def create_document_version(db: AsyncSession, payload: DocumentVersionCreate) -> DocumentVersionRecord:
    ...
    next_version_number = await document_versions_repository.get_latest_version_number(db, payload.document_id) + 1
    record = await document_versions_repository.create_document_version(db, payload, next_version_number)
